...
const models: Model[] = [
  { id: "granite-2b", name: "granite 3.0 2B" },
  { id: "granite-8b", name: "granite 3.0 8B" },
];
    ...
    <div className="flex items-start gap-2">
      {message.role === "assistant" && (
        <Avatar className="w-8 h-8 border">
          <AvatarImage src="/Logo-Matriv.svg" alt="AI Assistant Avatar" />
...
export default function AIChat() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/py/get_answer", {
        method: "POST",
        headers: {
